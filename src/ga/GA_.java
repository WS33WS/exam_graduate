...
public class GA_ {
	 ...
	 * 初始化一条染色体（用二进制字符串表示）
	 */
	private String initChr() {//随机生成长度为GENE的01字符串
		String res = "";
	...
	private double[] calculatefitnessvalue(String str) {
		...
		//substring(start,stop):位置！
		//start: 必需,一个非负的整数，规定要提取的子串的第一个字符在 stringObject 中的位置。
		//stop: 可选,一个非负的整数，比要提取的子串的最后一个字符在 stringObject 中的位置多 1。如果省略该参数，那么返回的子串会一直到字符串的结尾。
		//parseInt(string, radix):转换为radix进制数
		int a = Integer.parseInt(str.substring(0, 23), 2); //将a转换为23位的2进制数     
		int b = Integer.parseInt(str.substring(23, 46), 2);//将b转换为23位的2进制数 
	 ...
	 */
	private void select() {
		double evals[] = new double[ChrNum]; // 所有染色体适应值 10
	...
	public static void main(String args[]) {
		...
		GA_ Tryer = new GA_();
		Tryer.ipop = Tryer.initPop(); //产生初始种群，生成了ChrNum10个长度为GENE的二进制字符串
		String str = "";
